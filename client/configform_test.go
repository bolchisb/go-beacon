...
func testForm() *configForm {
	return newConfigForm(&resolved{
		Config:  Config{Server: "http://127.0.0.1:8080", AgentID: "target-01.example.com"},
...
func TestEnterSavesCurrentValues(t *testing.T) {
	...
	got := f.result()
	if got.Server != "http://127.0.0.1:8080" || got.AgentID != "target-01.example.com" {
...
func TestTabMovesFocusAndTypingEditsTheFocusedField(t *testing.T) {
	...
	if got := f.result().AgentID; got != "target-01.example.com-x" {
